      ...
      try {
        ...
        if (
          ...
          const isDefaultValue = (value, fieldName) => {
            ...
            if (
              fieldName === "featuredPrice" &&
              value === defaultFeaturedPrice
            ) {
              return true;
            }
          ...
          console.warn(
            "⚠️ No restaurants in API response. Response structure:",
            {
      ...
      filtered = filtered.filter((r) => {
        ...
        if (r.menu) {
          ...
          if (hasCategoryItem) {
            ...
            const categoryDish = getCategoryDishFromMenu(
              r.menu,
              selectedCategory,
            );
              ...
              return (
                ...
                >
                  {filter.hasIcon && filter.id === "price-match" && (
                    <span
                      className={`text-xs ${isActive ? "text-white" : "text-green-600 dark:text-green-400"}`}
                    >
                      ✓
                    </span>
                  )}
                  {filter.hasIcon && filter.id === "flat-50-off" && (
                    <span
                      className={`text-xs ${isActive ? "text-white" : "text-blue-500 dark:text-blue-400"}`}
                    >
                      ★
                    </span>
                  )}
                  <span
                    className={`text-sm font-bold ${isActive ? "text-white" : "text-black dark:text-white"}`}
                  >
                    {filter.label}
                  </span>
                </Button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-5 lg:gap-6">
            {filteredAllRestaurants.map((restaurant) => {
              ...
              return (
                <Link
                  key={restaurant.id}
                  to={`/user/restaurants/${restaurant.slug || restaurantSlug}`}
