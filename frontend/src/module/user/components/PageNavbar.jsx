...
export default function PageNavbar({
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        "🔄 Auto-triggering location fetch due to placeholder values",
      );
        ...
        requestLocation()
          .then((fetchedLocation) => {
            if (
              ...
              console.warn(
                "⚠️ Location fetch returned placeholder, user may need to select manually",
              );
            }
          })
          .catch((err) => {
  ...
  const locationDisplay = (() => {
    ...
    if (
      ...
      console.log(
        "✅✅✅ ZOMATO-STYLE: Using mainTitle for display:",
        mainLocation,
      );
    }
    ...
    if (
      ...
      const formattedParts = location.formattedAddress
        .split(",")
        .map((p) => p.trim())
        .filter((p) => p.length > 0);
      ...
      if (formattedParts.length >= 4) {
        console.log(
          "🔍 Using formattedAddress (complete address detected):",
          location.formattedAddress,
        );
        ...
        let cityIndex = -1;
        if (location?.city) {
          cityIndex = formattedParts.findIndex(
            (part) =>
        ...
        if (cityIndex === -1 && location?.state) {
          const stateIndex = formattedParts.findIndex(
            (part) =>
        ...
        if (cityIndex > 0) {
          ...
          if (localityParts.length > 0) {
            ...
            console.log(
              "✅✅✅ Extracted locality from complete formattedAddress:",
              mainLocation,
            );
          }
        } else {
          // If city not found, take first 3-4 parts (usually POI, building, floor, area)
          const localityParts = formattedParts.slice(
            0,
            Math.min(4, formattedParts.length - 2),
          );
          if (localityParts.length > 0) {
            mainLocation = localityParts.join(", ");
            console.log(
              "✅✅✅ Using first parts from formattedAddress:",
              mainLocation,
            );
          }
        }
    ...
    if (
      ...
      console.log(
        "🔍 Processing address field (Priority 2):",
        location.address,
      );
      ...
      const addressParts = location.address
        ...
        .map((p) => p.trim())
        .filter((p) => p.length > 0);
      const cityInAddress = addressParts.some(
        (part) =>
          part.toLowerCase() === location?.city?.toLowerCase() ||
      ...
      if (!cityInAddress && addressParts.length > 0) {
        ...
        let cityIndex = filteredParts.findIndex(
          (part) =>
            part.toLowerCase() === location?.city?.toLowerCase() ||
            part.toLowerCase() === "indore" ||
            part.toLowerCase() === "bhopal",
        ...
        if (cityIndex > 0) {
          ...
          console.log(
            "✅ Extracted locality from address field:",
            mainLocation,
          );
    ...
    if (
      ...
      console.log(
        "🔍 Processing formattedAddress (Priority 2):",
        location.formattedAddress,
      );
      ...
      console.log(
        "📋 Filtered parts (without pincode/country):",
        filteredParts,
      );
      ...
      if (location?.city) {
        cityIndex = filteredParts.findIndex(
          (part) => part.toLowerCase() === location.city.toLowerCase(),
        );
        console.log(
          `📍 City index (exact match with "${location.city}"):`,
          cityIndex,
        );
      }
      ...
      if (cityIndex === -1) {
        ...
        cityIndex = filteredParts.findIndex((part) =>
          commonCities.some(
            (city) => part.toLowerCase() === city.toLowerCase(),
          ),
        );
      ...
      if (cityIndex === -1 && location?.state) {
        const stateIndex = filteredParts.findIndex(
          (part) =>
            part.toLowerCase().includes(location.state.toLowerCase()) ||
            part.toLowerCase().includes("madhya") ||
            part.toLowerCase().includes("pradesh"),
      ...
      if (cityIndex > 0) {
        ...
        if (localityParts.length > 0) {
          mainLocation = localityParts.join(", ");
          console.log(
            "✅✅✅ Using exact locality from formattedAddress:",
            mainLocation,
          );
        } else {
        ...
        if (stateIndex > 0) {
          ...
          if (localityParts.length > 0) {
            mainLocation = localityParts.join(", ");
            console.log(
              "✅✅✅ Using parts before state from formattedAddress:",
              mainLocation,
            );
          }
        }
        ...
        if (!mainLocation && filteredParts.length >= 3) {
          mainLocation = filteredParts.slice(0, 3).join(", ");
          console.log(
            "✅✅✅ Using first 3 parts from formattedAddress (fallback):",
            mainLocation,
          );
        } else if (!mainLocation && filteredParts.length >= 2) {
          mainLocation = filteredParts.slice(0, 2).join(", ");
          console.log(
            "✅ Using first 2 parts from formattedAddress (fallback):",
            mainLocation,
          );
          ...
          if (!isCoordinates(firstPart) && firstPart.length > 2) {
            ...
            console.log(
              "✅ Using first part from formattedAddress (fallback):",
              mainLocation,
            );
          }
        }
    ...
    if (
      ...
      const filteredParts = parts.filter((part) => {
        ...
        if (part.toLowerCase() === "india" || part.length > 25) return false;
        if (
          part.toLowerCase() === "select location" ||
          part.toLowerCase() === "current location"
        )
          return false;
        return true;
      });
    ...
    if (
      ...
      console.log(
        "🔄🔄🔄 FORCE EXTRACTING from formattedAddress (last resort):",
        location.formattedAddress,
      );
      const parts = location.formattedAddress
        .split(",")
      ...
      if (filteredParts.length >= 3) {
        ...
        if (
          thirdPart === "indore" ||
          thirdPart === location?.city?.toLowerCase()
        ) {
          mainLocation = filteredParts.slice(0, 2).join(", ");
          console.log(
            "✅✅✅ FORCE: Using first 2 parts (3rd is city):",
            mainLocation,
          );
        } else {
          mainLocation = filteredParts.slice(0, 3).join(", ");
        ...
        if (
          !isCoordinates(firstPart) &&
          firstPart.length > 2 &&
          firstPart.toLowerCase() !== location?.city?.toLowerCase()
        ) {
          mainLocation = firstPart;
    ...
    // Skip if city is "Current Location" or "Select location" - these are placeholders
      ...
      console.log(
        "⚠️⚠️⚠️ FALLBACK: Using city (no locality found):",
        mainLocation,
      );
    }
    ...
    if (
      mainLocation &&
      (mainLocation.toLowerCase() === location?.city?.toLowerCase() ||
        mainLocation === "Indore")
    ) {
      console.log(
        "🔄🔄🔄 MainLocation is city, trying to extract locality one more time...",
      );
      ...
      if (
        ...
        if (cityIndex > 0) {
          ...
          if (areaParts.length > 0) {
            ...
            if (
              extractedArea &&
              extractedArea.toLowerCase() !== location.city.toLowerCase() &&
              !extractedArea.match(/^\d+/) &&
              extractedArea.length > 2 &&
              ...
              console.log(
                "✅✅✅ Extracted area from formattedAddress (before city):",
                mainLocation,
              );
              ...
              if (
                lastTwoParts.every(
                  (p) => p.toLowerCase() !== location.city.toLowerCase(),
                )
              ) {
                mainLocation = lastTwoParts.join(", ");
                console.log(
                  "✅✅✅ Extracted area (2 parts) from formattedAddress:",
                  mainLocation,
                );
              }
            }
          ...
          const filteredParts = parts.filter((part) => {
            if (/^\d{6}$/.test(part)) return false;
            if (part.toLowerCase() === "india" || part.length > 25)
              return false;
            if (
              part.toLowerCase() === "indore" ||
              part.toLowerCase() === location?.city?.toLowerCase()
          ...
          if (filteredParts.length >= 1) {
            ...
            if (
              ...
              mainLocation = `${extractedArea}, ${location.city}`;
              console.log(
                "✅✅✅ Extracted area (first part) from formattedAddress:",
                mainLocation,
              );
            }
          }
        ...
        if (parts.length >= 3) {
          ...
          for (let i = parts.length - 3; i >= 0; i--) {
            ...
            if (
              ...
              console.log(
                "✅✅✅ Last resort: Extracted area from formattedAddress:",
                mainLocation,
              );
              break;
            }
    ...
    if (hasCity && hasState) {
      ...
      if (location?.formattedAddress) {
        ...
        if (parts.length >= 4) {
          ...
          if (pincodeIndex > 1 && pincodeIndex !== -1) {
            ...
            console.log("📍 Extracted from pincode position:", {
              cityPart,
              statePart,
              pincodeIndex,
            });
            ...
            if (
              ...
              console.log(
                "✅ Using extracted city/state (pincode method):",
                subLocation,
              );
            }
          }
          ...
          if (!subLocation && parts.length >= 4) {
            ...
            console.log("📍 Trying India method:", {
              lastPart,
              secondLastPart,
            });
            ...
            if (lastPart === "India" && /^\d{6}$/.test(secondLastPart)) {
              ...
              console.log("📍 Extracted from India position:", {
                cityPart,
                statePart,
              });
              ...
              if (
                ...
                subLocation = `${cityPart}, ${statePart}`;
                console.log(
                  "✅ Using extracted city/state (India method):",
                  subLocation,
                );
              }
            }
          ...
          if (!subLocation && parts.length >= 6) {
            ...
            const pincodeIndex = parts.findIndex((part) =>
              /^\d{6}$/.test(part),
            );
            if (pincodeIndex === 6 && parts.length >= 7) {
              ...
              console.log("📍 Direct extraction (index method):", {
                cityPart,
                statePart,
                pincodeIndex,
              });
              ...
              if (
                ...
                subLocation = `${cityPart}, ${statePart}`;
                console.log(
                  "✅ Using extracted city/state (direct index method):",
                  subLocation,
                );
              }
            }
          ...
          if (!subLocation && parts.length >= 6) {
            ...
            if (
              ...
              subLocation = `${cityPart}, ${statePart}`;
              console.log(
                "✅ Using extracted city/state (simple fallback):",
                subLocation,
              );
            } else {
              console.log("⚠️ Validation failed for parts[4] and parts[5]:", {
                cityPart,
                statePart,
              });
            }
      ...
      if (
        ...
        const parts = location.address
          ...
          .map((part) => part.trim())
          .filter((part) => part.length > 0);
        console.log("📍 Trying address field:", {
          address: location.address,
          parts,
        });
        ...
        if (parts.length >= 4) {
          ...
          if (pincodeIndex > 1 && pincodeIndex !== -1) {
            ...
            if (
              ...
              subLocation = `${cityPart}, ${statePart}`;
              console.log(
                "✅ Using extracted city/state from address field:",
                subLocation,
              );
            }
          }
    ...
    if (
      subLocation &&
      (subLocation.includes("G-2") || subLocation.includes("Princess Center"))
    ) {
      ...
      if (location?.formattedAddress) {
        ...
        if (parts.length >= 6) {
          ...
          if (
            ...
            subLocation = `${cityPart}, ${statePart}`;
            console.log(
              "✅✅✅ FORCED extraction - New subLocation:",
              subLocation,
            );
          }
        }
