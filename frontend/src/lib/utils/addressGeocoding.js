...
export async function geocodeAddress(address, apiKey) {
  ...
  try {
    ...
    if (hasValidCoordinates) {
      console.log('✅ Address already has valid coordinates, skipping geocoding');
    ...
    if (!fullAddress || fullAddress.length < 5) {
      console.warn('⚠️ Address incomplete for geocoding:', fullAddress);
      return null;
    }
...
async function geocodeWithGoogleMaps(fullAddress) {
  return new Promise((resolve) => {
    ...
    geocoder.geocode({ address: fullAddress, region: 'in' }, (results, status) => {
      ...
      console.warn('⚠️ Geocoding failed (Google Maps API):', status);
      resolve(null);
    });
  });
}
...
async function geocodeWithHttpApi(fullAddress, apiKey) {
  try {
    ...
    if (!location?.lat || !location?.lng) {
      console.warn('⚠️ Geocoding invalid coordinates');
      return null;
    }
